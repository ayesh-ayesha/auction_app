...
class UserProfileRepository{
  ...
  UserProfileRepository(){
   userProfileCollection= FirebaseFirestore.instance.collection("userProfile");
  }



  Future<void> addUserProfile(UserProfile userProfile) async {
    await userProfileCollection.doc(userProfile.id).set(
    userProfile.toMap());
  }

  Future<UserProfile?> getUserById(String userProfileId) async {
    DocumentSnapshot snapshot=await userProfileCollection.doc(userProfileId).get();
  ...
  Future<List<UserProfile>> getAllUsers() async {
    try {
      ...
      List<UserProfile> users = querySnapshot.docs.map((doc) {
